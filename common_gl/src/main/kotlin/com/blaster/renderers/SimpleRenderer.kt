...
class SimpleRenderer(customPixelDecoder: PixelDecoder? = null) : Renderer  {
    ...
    override fun onDraw() {
        ...
        glBind(listOf(program, mesh, texture)) {
            program.setUniform(GLUniform.UNIFORM_VIEW_M, camera.calculateTransformM())
            program.setUniform(GLUniform.UNIFORM_PROJ_M, camera.projectionM)
            program.setUniform(GLUniform.UNIFORM_MODEL_M, node1.calculateTransformM())
            mesh.draw()
            program.setUniform(GLUniform.UNIFORM_MODEL_M, node2.calculateTransformM())
            mesh.draw()
        }
    }
}